...
The broker also retires itself when nobody needs it. After `LENS_BROKER_IDLE_EXIT_MS` (default 15 minutes; `0` disables) with no connected clients, no extension attached and no in-flight work, it releases the CDP lease and exits, and the next client spawns it again on demand. Any of those three conditions restarts the countdown. The extension guard matters: exiting while the extension is attached would drop its socket and push it back through broker rediscovery, which costs more than the resident process saves. The trade for idle exit is one extra Chrome **Allow** dialog on the CDP fallback path — consent is session-scoped, so a broker that has exited must ask again — in exchange for not squatting on the port and the debugging slot between sessions. `lens broker shutdown` retires it immediately.

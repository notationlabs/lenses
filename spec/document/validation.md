...
- **Default agrees with declaration:** A declared `default` must match the parameter's type (`integer` requires an integer) and, when `enum` is present, be one of the enum values.

...
- **No-browser exit:** With no connected client and nothing in flight, a broker that can reach no browser exits after `LENS_BROKER_NO_BROWSER_EXIT_MS` (default 10 s).
...
- **Two CDP transports:** The broker prefers Playwright Extension (copied CDP relay → synthetic endpoint → Puppeteer) and falls back to direct Chrome remote debugging. Page sessions, intercepts, screenshots, and auth-gate bookkeeping are shared. Playwright Extension only sees tabs in its client tab group; `chrome.debugger` stays attached and Chrome shows a debugger infobar.
- **Status:** Status reports the selected backend and capabilities plus every backend's availability and diagnostics. Advice names Playwright Extension install/connect vs CDP Allow.
...
- **Extension disconnect:** If the Playwright Extension relay drops, calls fail on that transport and the broker can use direct CDP. An in-progress session is not silently replayed on the other transport.
